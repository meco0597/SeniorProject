...
BATMANMessage = robotCommands.robotMessage("Read", 250, 0)


def messageHandler(data, addr):
    print(data, addr)

server = udpServer(5050, localIP, messageHandler)
...
def Main():
    ...
    while True:
        ...
        #time.sleep(0.1)
        #server.sendMessage(forwardMessage, RobotIP3, udpPort)
        #time.sleep(waitTime / 1000.0)
        #server.sendMessage(reverseMessage, RobotIP, udpPort)
        #time.sleep(waitTime / 1000.0)
        #server.sendMessage(stopMessage, RobotIP3, udpPort)
        server.sendMessage(BATMANMessage, RobotIP, udpPort)
        time.sleep(waitTime / 1000.0)
